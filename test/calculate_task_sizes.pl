...
my $plugin_json = read_file ('../cudaHashcat.json') or die "cannot read cudaHashcat modes file!\n";
...
foreach my $format (@{$$plugin{'algorithms'}})
{
    ...
    my $speed = `cudaHashcat64.bin -d 1 -w 3 -m $mode '$hash' $wordlist -r rules/rockyou-30000.rule --status-automat --runtime 7 | sed -rn 's/.*PROGRESS\\t([0-9]*)[^0-9].*/\\1/p' | tail -n 1`;
